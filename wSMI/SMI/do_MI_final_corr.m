function time = do_MI_final_corr(file,fileout)
tic
...
%     chann_sel = 1:2;
    taus = [1 2 3 4 5]; %% corresponds to 2 4 8 16 32
...
%        disp([file ' - tau:' num2str(2^(tau-1)) ', trial ' num2str(trial) ' of ' num2str(size(sym{tau},3))])
   ...
   [AUX1(:,trial),AUX2(:,trial),AUX3(:,trial),AUX4(:,trial)] = MI_SMI_and_wSMI_diag(sym{tau}(:,:,trial),count{tau}(:,:,trial),3);
