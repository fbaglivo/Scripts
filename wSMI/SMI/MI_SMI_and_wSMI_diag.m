function [SMI_out, wSMI_out,R_SMI_out, R_wSMI_out] = MI_SMI_and_wSMI_diag(signal,sym_prob,kernel)
...
for ch1 = 1:(channels-1) %%% from channel 
    ...
    for ch2 = (ch1+1):channels %%% to channel 
        ...
        for xxx = 1:n_symbols^2 %Calculate joint probability
            joint_prob(xxx,index) = sum(meta_signal == xxx)/samples;
        end
        
%%
        
        jp=reshape(joint_prob,6,6);
